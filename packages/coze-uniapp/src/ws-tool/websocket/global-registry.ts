...
export function registerWebSocketFactory(factory: {
  ...
  if (typeof window !== 'undefined') {
    (window as unknown as Record<string, unknown>).UniAppWebSocketFactory = factory;
  }

  // 在全局作用域中定义获取函数，确保小程序环境可访问
  (globalThis as unknown as Record<string, unknown>).getMiniAppWebSocketFactory = getWebSocketFactory;
}
